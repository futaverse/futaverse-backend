        ...
        if not user:
            ...
            if existing_user:
                raise exceptions.AuthenticationFailed(
                    detail="Incorrect password.", code="incorrect_password"
                )
...
class StudentProfileSerializer(serializers.ModelSerializer):
    ...
    profile_img = serializers.SlugRelatedField(
        queryset=UserProfileImage.objects.all(), required=False, slug_field="sqid"
    )
...
class AlumniProfileSerializer(serializers.ModelSerializer):
    previous_comps = serializers.ListField(
...
class PersonSearchResultSerializer(serializers.ModelSerializer):
    ...
    def _get_profile(self, obj):
        ...
        return getattr(obj, "alumni_profile", None)
    ...
    def get_middlename(self, obj):
        profile = self._get_profile(obj)
        return profile.middlename if profile else None

    @extend_schema_field(serializers.URLField(allow_null=True))
    def get_profile_img_url(self, obj):
        image = obj.profile_img.order_by("-uploaded_at").first()
        return image.image.url if image else None
