...
from core.serializers import UserInfoSummarySerializer
...
class CreateTicketSerializer(serializers.ModelSerializer):
    event = serializers.SlugRelatedField(
        queryset=Event.objects.all(), slug_field="sqid"
    )
...
class EventSerializer(serializers.ModelSerializer):
    ...
    platform = serializers.ChoiceField(
        choices=VirtualMeeting.Platform, required=False, write_only=True
    )
    ...
    def validate(self, attrs):
        ...
        free_ticket_data = validated_data.get("free_ticket")
        paid_tickets_data = validated_data.get("tickets", [])

        if (not free_ticket_data or not free_ticket_data.get("required")) and len(
            paid_tickets_data
        ) == 0:
            raise serializers.ValidationError(
                {"detail": "At least one ticket (free or paid) is required for events"}
            )

        if (mode == Event.Mode.VIRTUAL or mode == Event.Mode.HYBRID) and not platform:
            raise serializers.ValidationError(
                {
                    "platform": "Platform is required for events with virtual or hybrid modes"
                }
            )
        ...
        if len(paid_tickets_data) > 0:
            ...
            if (
                not account_details
                or not account_details.is_active
                or not account_details.subaccount_code
            ):
                raise serializers.ValidationError(
                    {
                        "detail": "Your account is not set up to receive payments. Please add your bank details to create paid tickets so you can receive payments."
                    }
                )
    ...
    def create(self, validated_data):
        ...
        free_ticket_data = validated_data.pop("free_ticket", None)

        event = super().create(validated_data)

        if free_ticket_data and free_ticket_data.get("required"):
            Ticket.objects.create(
                ...
                quantity=free_ticket_data.get("quantity"),
            )

        if tickets_data:
            for ticket_data in tickets_data:
                Ticket.objects.create(
                    event=event, **ticket_data, type=Ticket.Type.CUSTOM
                )
...
class TicketPurchaseSerializer(serializers.ModelSerializer):
    ticket = serializers.SlugRelatedField(
        queryset=Ticket.objects.all(), slug_field="sqid"
    )
    ...
    def validate_ticket(self, value: Ticket):
        ...
        if value.sales_start > timezone.now():
            raise serializers.ValidationError(
                {"ticket": "Ticket sales have not started yet"}
            )
...
class UpdateEventSerializer(serializers.ModelSerializer):
    ...
    def validate_max_capacity(self, value):
        ...
        if value and value < total_sold:
            raise serializers.ValidationError(
                f"Capacity cannot be lower than already sold tickets ({total_sold})."
            )
        return value
    ...
    def validate(self, attrs):
        ...
        if hasattr(self.instance, "virtual_meeting"):
            platform = validated_data.get(
                "platform",
                self.instance.virtual_meeting.platform if self.instance else None,
            )

        if mode in [Event.Mode.PHYSICAL, Event.Mode.HYBRID] and not venue:
            raise serializers.ValidationError(
                {"venue": "Venue is required for Physical or Hybrid events."}
            )

        if mode in [Event.Mode.VIRTUAL, Event.Mode.HYBRID] and not platform:
            raise serializers.ValidationError(
                {"platform": "Platform is required for Virtual or Hybrid events."}
            )
